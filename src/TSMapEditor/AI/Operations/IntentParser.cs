...
4. 创建完整对战地图（必须严格按此顺序）：
   ...
   - 第二步：用 set_height 创建地形起伏（见下方高度规则）
   - 第三步：用 set_waypoint 设置玩家出生点（对称分布，放在平地height=0处）
   - 第四步：在出生点附近建基地（使用多个 place_building，放在平地处）
   - 第五步：用 place_overlay 在出生点附近铺矿区（★ 必须远离建筑 5+ 格）
   - 第六步：用 place_terrain_object 散布装饰物（树/石，避开基地和矿区）
   - 注意地图对称性（对战公平性）

=== 高度设置规则（极其重要）===

set_height 必须遵守以下规则，否则会出现丑陋的黑色悬崖断裂：
- 相邻格子之间高度差最多为 1（比如从0到1到2，不能直接0到3）
- 正确做法：使用多个 set_height 创建渐变坡度，例如：
  中心 height=4，周围一圈 height=3，再外一圈 height=2，最外 height=1
  示例：一个丘陵应该是 4 层同心矩形：
    set_height (50,50) 20x20 height=1  ← 最外层
    set_height (53,53) 14x14 height=2  ← 中间层  
    set_height (56,56) 8x8 height=3    ← 内层
    set_height (58,58) 4x4 height=4    ← 山顶
- ★ 建筑和出生点必须放在 height=0 的平地上
- 高地主要用于地图中央或角落的装饰性地形，不要在出生点附近设置高地

重要提示：
- 对于复杂任务，可以返回很多个 operations（10-100个都正常）
- 建筑之间至少留 3-4 格间距，否则会重叠
- 矿区和建筑区域之间至少间隔 5 格
