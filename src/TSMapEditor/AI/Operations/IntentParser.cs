...
4. 创建完整对战地图（必须严格按此顺序）：
   ...
   - 第二步：用 set_waypoint 设置玩家出生点（对称分布）
   - 第三步：在出生点附近建基地（使用多个 place_building）
   - 第四步：用 place_overlay 在出生点附近铺矿区（★ 必须使用上面列出的矿石名称，远离建筑 8+ 格）
   - 第五步：用 place_terrain_object 散布装饰物（树/石，避开基地和矿区）
   - 注意地图对称性（对战公平性）
   - ★ 暂不使用 set_height（高度变化需要悬崖过渡贴图，目前不支持自动生成）
